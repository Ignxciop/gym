...
interface ProfileSectionProps {
    user: { name: string; email?: string; birthDate?: string | null; avatarUrl?: string | null; gender?: string | null };
    userData: any[];
    setUserData: (data: any[]) => void;
    setUser?: (u: any) => void;
}

const ProfileSection: React.FC<ProfileSectionProps> = ({ user, userData, setUserData, setUser }) => {
    ...
    // Avatar y género: sincronizar con user
    // Si avatarUrl es relativa, prepende el origin para mostrar correctamente la imagen
    const getAvatarUrl = (u: any) => {
        if (u.avatarUrl && typeof u.avatarUrl === "string" && u.avatarUrl.trim() !== "") {
            ...
            // Si estamos en el navegador, usar window.location.origin
    ...
    const [gender, setGender] = useState(getGender(user));
    const [genderSuccess, setGenderSuccess] = useState("");
    const [genderError, setGenderError] = useState("");
    const [savingGender, setSavingGender] = useState(false);
    const [genderSaved, setGenderSaved] = useState(getGenderSaved(user));

    // Sincronizar avatar y género si cambia el usuario (objeto completo)
    React.useEffect(() => {
    ...
    const handleBirthDateSave = async (updatedUserData: any) => {
        ...
        // Refrescar datos del usuario (incluyendo birthDate, avatarUrl, gender) si setUser está disponible
        if (setUser && user.email) {
    ...
    const handleUserDataSuccess = (newData: any) => {
        // Si la API no retorna createdAt, lo agregamos manualmente como string ISO
        if (!newData.createdAt) {
            newData.createdAt = new Date().toISOString();
        } else if (typeof newData.createdAt === 'number') {
            // Si por alguna razón es un número, convertirlo a string ISO
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (setUser && user.email) {
                        ...
                        // Actualizar preview si avatarUrl viene del backend
    ...
    const handleGenderSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setGenderSaved(true);
            // Refrescar datos y bloquear selección
            if (setUser && user.email) {
                const res = await fetch(`/api/userinfo?email=${user.email}`);
                const userInfo = await res.json();
                setUser({
    ...
    // Aquí deberías retornar el JSX real de tu sección de perfil, por ejemplo:
    return (
            ...
            <table className="min-w-full bg-gray-900 rounded-xl shadow border border-green-700">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-3 text-green-400 text-lg font-bold text-left">Edad</th>
                  <th className="px-4 py-3 text-green-400 text-lg font-bold text-left">Estatura (cm)</th>
                </tr>
              </thead>
              <tbody>
                {userData.map((row, idx) => {
                  // Formatear fecha a dd-mm-aaaa usando solo row.date
                  let fecha = '--';
                  let d = null;
                  ...
                  return (
                    <tr key={row.id || idx} className="border-t border-green-800 hover:bg-gray-800/60 transition">
                      ...
                      <td className="px-4 py-2 text-green-200 text-base font-semibold">{row.age ? `${row.age} años` : '--'}</td>
                      <td className="px-4 py-2 text-green-200 text-base font-semibold">{row.height ?? '--'}</td>
                    </tr>
                  );
                })}
              </tbody>
